...
async function loadDocumentos() {
  await fetchHouses();
  const docs = await api(rest('documentos_casa', 'select=id,patrimonio_id,nome_arquivo,tipo_arquivo,tamanho,caminho_arquivo,descricao,data_upload&order=data_upload.desc'));
  state.documentos = docs;
  const assets = new Map(state.houses.map(asset => [String(asset.id), asset]));
  const documentsFor = assetId => docs.filter(doc => String(doc.patrimonio_id) === String(assetId));
  const documentItem = doc => `<article class="document-item"><div class="document-file-icon" aria-hidden="true">▤</div><div class="document-file-info"><strong title="${esc(doc.nome_arquivo)}">${esc(doc.nome_arquivo)}</strong><span>${esc(doc.descricao || fileType(doc.tipo_arquivo) || 'Sem descrição')}</span><small>${fileType(doc.tipo_arquivo)} · ${Math.ceil(Number(doc.tamanho || 0) / 1024)} KB · ${dateTime(doc.data_upload)}</small></div><div class="action-row"><button class="icon-button" data-open-doc="${doc.id}">Abrir</button><button class="icon-button delete" data-delete-doc="${doc.id}">Excluir</button></div></article>`;
  const folder = (asset, folderDocs, removed = false) => `<details class="document-folder" ${folderDocs.length ? 'open' : ''}><summary><span class="document-folder-icon" aria-hidden="true">▰</span><span><strong>${esc(removed ? 'Patrimônio removido' : assetLabel(asset))}</strong><small>${folderDocs.length === 1 ? '1 documento' : `${folderDocs.length} documentos`}</small></span><span class="document-folder-chevron" aria-hidden="true">⌄</span></summary><div class="document-folder-content">${folderDocs.length ? folderDocs.map(documentItem).join('') : '<p class="detail-empty">Esta pasta ainda não possui documentos.</p>'}</div></details>`;
  const assetFolders = state.houses.map(asset => folder(asset, documentsFor(asset.id))).join('');
  const orphanDocs = docs.filter(doc => !assets.has(String(doc.patrimonio_id)));
  const folders = assetFolders + (orphanDocs.length ? folder(null, orphanDocs, true) : '');
  app.innerHTML = heading('Arquivos', 'Documentos', 'Armazene documentos organizados nas pastas de cada patrimônio.') + `<section class="card form-card"><h2 class="card-title">Enviar documento</h2><form id="document-form" class="form-grid two"><div class="field"><label>Patrimônio *</label><select name="patrimonio_id" required><option value="">Selecione</option>${assetOptions()}</select></div><div class="field"><label>Arquivo *</label><input name="file" type="file" accept="application/pdf,image/png,image/jpeg" required></div><div class="field"><label>Descrição</label><input name="descricao"></div><div class="form-actions"><button class="button">Enviar arquivo</button></div></form><p class="upload-note">PDF, PNG ou JPEG. O espaço disponível depende do plano da conta.</p></section><section class="document-folders"><div class="table-head"><div><h2>Pastas dos patrimônios</h2><p>${docs.length === 1 ? '1 documento armazenado' : `${docs.length} documentos armazenados`}</p></div><button id="reload" class="button secondary small">Atualizar</button></div>${folders || '<section class="card empty">Cadastre um patrimônio para criar a primeira pasta de documentos.</section>'}</section>`;
  document.querySelector('#document-form').onsubmit = uploadDocument;
  document.querySelector('#reload').onclick = refresh;
  app.querySelectorAll('[data-open-doc]').forEach(button => button.onclick = () => openDocument(button.dataset.openDoc));
  app.querySelectorAll('[data-delete-doc]').forEach(button => button.onclick = () => deleteDocument(button.dataset.deleteDoc));
}
